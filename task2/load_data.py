...
import random
from visualize import visual_4
...
class CTDataset1:
    def __init__(self, config, mode, horizontal_only=True, model=None):
        ...
        if isinstance(self.data_info[0], str):
            ...
            # self.img_lst = [data['image'] for data in self.data_info[1:13]]
            # self.label_lst = [data['label'] for data in self.data_info[1:13]]
        self.total_images = []
        self.total_labels = []
        self.centers = []
        self.bboxes = []
    ...
    def get_grid_points(self, idx):
        with torch.no_grad():
            ...
            # points_2 = points_0 + points_1
            # save_path = os.path.join(self.config['log_dir'], self.config['exp'], self.config['time_stamp'] + '-main', 'tmp.png')
            # visual(self.total_images[idx], label, np.array(points_0 + points_1), np.array([0] * len(points_0) + [1] * len(points_1)), path=save_path)
            # exit()
            # pointsx = points_0[:len(points_1)] + points_1
            # points = torch.tensor(pointsx).to(model.device).unsqueeze(0)
            # point_labels = torch.tensor([label[x, y] for x, y in pointsx]).to(model.device).unsqueeze(0)
