...
class SAM_finetune():
    ...
    def train_step(self, data_feed, epoch, step):
        ...
        return loss, foc_loss, dic_loss, mask_loss, iou_loss
    ...
    def train(self, train_loader, val_loader):
        ...
        for epoch in range(self.config['max_epoch']):
            for step, data_feed in enumerate(train_loader):
                if step % 100 == 0:
                    avg_loss, avg_mask_loss, avg_iou_loss = self.validate(val_loader)
                    self.writer.add_scalar('val/loss', avg_loss, global_step=(epoch+1)*len(train_loader)+step)
                    self.writer.add_scalar('val/loss', avg_mask_loss, global_step=(epoch+1)*len(train_loader)+step)
                    self.writer.add_scalar('val/loss', avg_iou_loss, global_step=(epoch+1)*len(train_loader)+step)
                loss, foc_loss, dic_loss, mask_loss, iou_loss = self.train_step(data_feed, epoch + 1, step + 1)
    ...
    def validate(self, val_loader):
        ...
        if m_dice > self.best:
            self.best = m_dice
            self.save_model(-1)
        return avg_loss, avg_mask_loss, avg_iou_loss
